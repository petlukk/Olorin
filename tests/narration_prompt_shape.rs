...
#[test]
fn long_output_still_builds_a_prompt() {
    // Length no longer gates narration: the old byte cap silenced good long
    // prose and json too. A long, even repetitive, output now produces a
    // prompt; the only real failure mode (grid continuation) is caught
    // post-generation by runes::narration::is_grid_continuation, not here.
    ...
    assert!(data.len() > 600, "fixture should be a long output; got {}", data.len());

    let r = make_result(&data);
    let p = build_narration_prompt("eatime", OutputSafety::Trusted, r);
    assert!(
        p.is_some(),
        "long output must still build a narration prompt; got None"
    );
}

#[test]
fn short_output_still_narrates() {
    let data = "rows: 5\ncolumn_a: mean=42\ncolumn_b: 7 unique\n";
    let r = make_result(data);
