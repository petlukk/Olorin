//! Step 0–4: Gemma 4 forward-pass setup verification.
//!
//! Step 0 loads the GGUF and validates structural metadata. Steps 1–4 walk
//! through embedding lookup, attn_norm, QKV projection (with per-head Q/K
//! norm + RoPE) and PLE Phase A — the foundational ops that must produce
//! sane intermediates before later parity tests can be trusted.
//!
//! Run: cargo test --release --test gemma4_verify -- --nocapture
//!
//! Logit and per-token output comparison vs llama.cpp lives in
//! `gemma4_logits_vs_llama.rs`. Quant kernel parity tests (Q8K/Q4K/Q5K/Q6K)
//! live in `quant_kernels_vs_llama.rs`. Op kernel parity tests (RMSNorm,
//! RoPE, GELU, softmax/softcap, f32↔f16) live in `ops_vs_llama.rs`.
//!
...
fn step3_qkv_projection() {
    ...
    // RoPE at pos=0 (cos=1, sin=0 — should not change values)
