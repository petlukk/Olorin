//! Bit-exact correctness gate for q5k_8x8_q8k_gemm (ARM only).
//!
//! Reference: per-(token, row) q5k_dot_q8k against un-repacked weights.
//! Implementation: q5k_repack_8x8 then q5k_8x8_q8k_gemm against
//! block_q8_Kx4 repacked activations.
//!
//! Tests N in {4, 8, 16, 32} against layer-0 wk (Q5K).
...
#[test]
fn q5k_8x8_gemm_matches_matvec_loop_bitexact() {
...
fn inner() {
    ...
    for &batch_n in &[4, 8, 16, 32] {
        ...
        unsafe {
            olorin::kernels::ffi_inference::q5k_8x8_q8k_gemm(
                repacked.as_ptr(),
                q8_a.as_ptr(),
                scratch.as_mut_ptr(),
                gemm_out.as_mut_ptr(),
