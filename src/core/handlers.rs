...
pub fn dispatch_tool_call(name: &str, input: &crate::storage::json::Object) -> Result<String, String> {
    ...
    //
    // Args contract is v1 single-primary-string. If the Object has exactly one
    // field and its value is a string, extract that string as the args value —
    // tools/runes take free text (e.g. "/tmp/x.csv"), not JSON like
    ...
    // 1) Tools path.
    if let Some(tool_result) = crate::tools::run_tool(name, &args) {
        if !tool_result.success {
            return Err(tool_result.output);
        }
    ...
    // 2) Runes path.
    if let Some(result) = crate::runes::run_rune(name, &args) {
