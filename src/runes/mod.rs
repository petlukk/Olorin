...
///
/// Returns `None` only when the output is structured (`--json`) or blocked by
/// the inbound safety scan — the caller then skips narration. Output LENGTH is
/// no longer gated here: long prose and json narrate fine on the production
/// quant, and the one real failure mode (the model continuing a long numeric
/// grid instead of summarizing) is caught after the fact by
/// [`narration::is_grid_continuation`]. Discarding a garbled narration once we
/// can see it beats guessing from a byte count that mis-fired both ways.
///
