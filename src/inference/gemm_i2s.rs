...
pub(crate) fn i2s_gemm_mt(
    ...
    let qs_ptr = SendPtr(qs.as_ptr());
    let scales_ptr = SendPtr(scales.as_ptr());
    let sums_ptr = SendPtr(sums.as_ptr());
    ...
    pool.run(total, move |tid, _n| {
        ...
        let mut r = start;
        unsafe {
            let qs = std::slice::from_raw_parts(qs_ptr.ptr(), nt);
            let scales = std::slice::from_raw_parts(scales_ptr.ptr(), nt);
            let sums = std::slice::from_raw_parts(sums_ptr.ptr(), nt);
            while r + 4 <= end {
