...
/// Q3K 8×8 repacked batch GEMM: work-stealing across output row tiles.
/// Mirrors `q4k_gemm_8x8_batch_ws` but for Q3K (block_q3_Kx8, 1168 B/sb tiles).
/// ARM-only — no x86 `q3k_8x8_q8k_gemm` kernel exists.
///
/// `packed` must be pre-repacked via `q3k_repack_8x8`.
/// `q8_a` must be pre-repacked into block_q8_Kx4 format via `q8k_repack_4`.
/// `nr` = number of tokens (must be multiple of 4, caller zero-pads).
/// `nc` = number of output rows (total, must be multiple of 8).
#[cfg(target_arch = "aarch64")]
#[allow(clippy::too_many_arguments)]
pub fn q3k_gemm_8x8_batch_ws(
    packed: *const u8,
    q8_a: *const u8,
    output: *mut f32,
    n_inner: usize,
    ...
    let nb = n_inner / 256;
    let tile_bytes = nb * 1168; // block_q3_Kx8 tile size
    let n_tiles = nc / 8;
    ...
    while (chunk as usize) < n_tiles {
        ...
        unsafe {
            ffi_inference::q3k_8x8_q8k_gemm(
                w_ptr,
                q8_a,
                scratch.as_mut_ptr(),
                output.add(col_start),
