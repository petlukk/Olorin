...
fn load_q4k_tensor(gguf: &GgufFile, name: &str) -> Result<(*const u8, usize)> {
    let data = gguf
        .tensor_data(name)
        .ok_or_else(|| Error::Inference(format!("missing tensor: {name}")))?;
...
fn load_qk_tensor(gguf: &GgufFile, name: &str) -> Result<(*const u8, usize, usize)> {
...
fn tensor_ptr<T>(gguf: &GgufFile, name: &str) -> Result<*const T> {
    let data = gguf
        .tensor_data(name)
        .ok_or_else(|| Error::Inference(format!("missing tensor: {name}")))?;
...
fn load_i2s_tensor(
    ...
    let data = gguf
        ...
        .ok_or_else(|| Error::Inference(format!("missing tensor: {name}")))?;
    if data.len() < 32 {
        return Err(Error::Inference(format!("{name}: tensor too small for I2_S scale")));
    }
...
impl BitNetModel {
    ...
    pub fn from_gguf(gguf: &GgufFile) -> Result<Self> {
        ...
        let n_layers = get_meta_u32(gguf, &key("block_count"))
            .ok_or_else(|| Error::Inference("missing metadata: block_count".into()))? as usize;

        let hidden_dim = get_meta_u32(gguf, &key("embedding_length"))
            .ok_or_else(|| Error::Inference("missing metadata: embedding_length".into()))? as usize;

        let n_heads = get_meta_u32(gguf, &key("attention.head_count"))
            .ok_or_else(|| Error::Inference("missing metadata: head_count".into()))? as usize;
        ...
        let ffn_dim = get_meta_u32(gguf, &key("feed_forward_length"))
            .ok_or_else(|| Error::Inference("missing metadata: feed_forward_length".into()))? as usize;
