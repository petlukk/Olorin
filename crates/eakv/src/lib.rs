...
extern "C" {
    ...
    pub fn eakv_cache_save(cache: *const eakv_cache_t, path: *const std::ffi::c_char) -> c_int;
    pub fn eakv_cache_load(path: *const std::ffi::c_char, out: *mut *mut eakv_cache_t) -> c_int;
}
...
impl EakvCache {
    ...
    pub fn save(&self, path: &str) -> Result<(), String> {
        ...
        let rc = unsafe { eakv_cache_save(self.ptr, c_path.as_ptr()) };
        if rc == EAKV_OK { Ok(()) } else { Err(format!("eakv_cache_save failed: {rc}")) }
    }

    pub fn load(path: &str) -> Result<Self, String> {
        let c_path = std::ffi::CString::new(path).map_err(|e| e.to_string())?;
        let mut ptr: *mut eakv_cache_t = std::ptr::null_mut();
        let rc = unsafe { eakv_cache_load(c_path.as_ptr(), &mut ptr) };
        if rc == EAKV_OK && !ptr.is_null() {
            Ok(Self { ptr, n_layers: 0, n_kv_heads: 0, head_dim: 0 })
        } else {
            Err(format!("eakv_cache_load failed: {rc}"))
        }
    }
}
