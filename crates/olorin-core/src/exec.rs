...
pub fn run(argv: &[&str]) -> io::Result<Output> {
    ...
    // Parent: close write ends, read from child
    unsafe {
        libc::close(stdout_pipe[1]);
        libc::close(stderr_pipe[1]);
    }
...
/// A spawned child process with piped stdin/stdout for long-lived subprocesses.
pub struct Child {
    pub pid: i32,
    pub stdin_fd: i32,
    pub stdout_fd: i32,
...
pub fn spawn(argv: &[&str]) -> io::Result<Child> {
    ...
    if pid == 0 {
        unsafe {
            libc::close(stdin_pipe[1]);  // close parent's write end
            libc::close(stdout_pipe[0]); // close parent's read end
            libc::dup2(stdin_pipe[0], 0);  // child stdin
            libc::dup2(stdout_pipe[1], 1); // child stdout
            libc::close(stdin_pipe[0]);
            libc::close(stdout_pipe[1]);
            libc::execvp(c_ptrs[0], c_ptrs.as_ptr());
            libc::_exit(127);
    ...
    // Parent
    unsafe {
        libc::close(stdin_pipe[0]);  // close child's read end
        libc::close(stdout_pipe[1]); // close child's write end
    }
